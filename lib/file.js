...
function getReleaseExt(ext){
    if(ext && fis.cli.commander.args[0].lc){
...
function getDomain(path){
    var hash = fis.util.md5(path),
        ...
        domain = '';
    if(len){
        var deploy = fis.cli.commander.args[0].dest;
        if(deploy.indexOf('test') > -1){
            domain = domains[1];
        }else if(deploy.indexOf('dev') > -1){
            domain = domains[0];
        }else{
            domain = domains[2];
        }
...
var File = Object.derive(function(){
    ...
    if(_.isTextFile(rExt)) {
        ...
        if(rExt === fis.config.get('project.md5Postfix') + '.js'){
            this.isJsLike = true;
            this.useDomain = true;
            this.useHash = true;
            this.useMap = true;
        }else if(rExt === fis.config.get('project.md5Postfix') + '.css'){
            this.isCssLike = true;
            this.useDomain = true;
            this.useHash = true;
            this.useMap = true;
    ...
    if(realpath.indexOf(root) === 0){
        ...
        if(this.release === false){
            ...
            Object.defineProperty(this, 'url', {
                enumerable : true,
                get : function(){
                    fis.log.warning('文件不存在，unreleasable file [' + self.realpath + ']');
                }
            });
        } else {
    ...
    getHashRelease : function(release){
        ...
        if(release){
            ...
            fis.log.warning('文件不存在，unreleasable file [' + this.realpath + ']');
        }
    },
    ...
    addDependency : function(id){
        try{
            if(id && (id = id.trim())){
                if(this.dependency.indexOf(id) < 0){
                    this.dependency.push(id);
                }
                return id;
            }
            return false;
