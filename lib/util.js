...
_.md5 = function(data, len){
    ...
    var dd = md5sum.digest('hex');
    // 默认取md5值前8位
    if(fis.config.get('project.md5Afterbody')){
        return dd.substring(dd.length - len); // 取后8位
    }else{
        return dd.substring(0, len); // 取前8位
    }
};
...
_.pipe = function(type, callback, def){
    type = type.replace(fis.config.get('project.md5Postfix'), '');
